...
^if($mode eq "html"){
	^if($type eq "separator"){
		...
		^if($type ne "current" && def $page_num){
			^if(^url.left(11) eq "javascript:"){
				$result[<a href="javascript:dummy()" onClick="^url.match[{page_num}][g]{$page_num}">$name</a>]]
			}{
				$result[<a href="^untaint[html]{^if(def $url){$url}{./}^if($page_num != $first_page){${url_separator}$form_name=$page_num}}">$name</a>]]
			}
		}{
			^if(^url.left(11) eq "javascript:"){
				$result[<a class="active" href="javascript:dummy()" onClick="^url.match[{page_num}][g]{$page_num}">$name</a>]]
			}{
				$result[<a class="active" href="^untaint[html]{^if(def $url){$url}{./}^if($page_num != $first_page){${url_separator}$form_name=$page_num}}">$name</a>]]
			}
		}
	}
}{
